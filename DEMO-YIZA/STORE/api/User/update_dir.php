<?php 
	...
	(strpos($municipio, '<') !== false || strpos($municipio, '>') !== false) 
		...
		$res = $conexion->query($sqlC);
		$num = $res->num_rows;
		if ($num == 1) {
			$sql = "UPDATE direccion SET no_exterior = '$nex', no_interior = '$nix', cod_postal = $cp, id_estados = $estado, colonia = '$colonia', calle = '$calle', municipio = '$municipio' WHERE id_usu = $id_usu";
			...
			$response->state=false;
			$response->detail="El no tiene dirección, primero agregala";
		}
