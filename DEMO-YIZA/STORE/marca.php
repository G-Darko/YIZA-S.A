...
<head>
	...
	<title><?php echo $rMar['nombre'];?> | <?php echo $mTitle;?></title>
...
<body>
	<div class="contenedor">
		<div class="main"> -->
			...
			<div onclick="p()">
			<h1 class="titulo"><?php echo $rMar['nombre'];?></h1>
		   	<div class="productos">
		   		<?php 

		   		$sqlT = mysqli_query($conexion, "SELECT COUNT(*) as total FROM productos WHERE id_statusProd = 1 AND precio > 0 AND id_mar = $id_mar");
				$resT = mysqli_fetch_assoc($sqlT);
				$total = $resT['total'];

				$sqlTM = mysqli_query($conexion, "SELECT COUNT(*) as totalM FROM marcas");
				$resTM = mysqli_fetch_assoc($sqlTM);
				$totalM = $resTM['totalM'];
				...
				if ($page <= 0 || $_GET['page'] <= 0 || ($page > $tPags && $page != 1) || ($_GET['page'] > $tPags && $_GET['page'] != 1) ) {
						header("Location: marca.php?id=1&page=1");
				}


		   			$prod = "SELECT * FROM productos WHERE id_statusProd = 1 AND precio > 0 AND id_mar = $id_mar LIMIT $desde, $xPage";
		   	...
		   	<div class="paginador">
					...
					<?php 
						if ($page != 1) {
					?>
							<li><a href="marca.php?id=<?php echo $id_cat;?>&page=1">|<</a></li>
							<li><a href="marca.php?id=<?php echo $id_cat;?>&page<?php echo $page-1;?>"><<</a></li>
					<?php 
						}
							if ($tPags <= 5) {
								for ($i=1; $i <= $tPags; $i++) { 
									if ($i == $page) {
										...
										echo '<li><a href="marca.php?id='.$id_cat.'&page='.$i.'">'.$i.'</a></li>';
									}
								}
							}else{
					...
					?>
							<li><a href="marca.php?id=<?php echo $id_cat;?>&page=<?php echo $page+1;?>">>></a></li>
							<li><a href="marca.php?id=<?php echo $id_cat;?>&page=<?php echo $tPags;?>">>|</a></li>
					<?php 
						}
					?>
						</ul>
